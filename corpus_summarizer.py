...
class CustomClusterFeatures(ClusterFeatures):
    def __init__(
        self,
        features: ndarray,
        ...
        super(CustomClusterFeatures, self).__init__(features, algorithm, pca_k, random_state)


class CustomSummaryProcessor(SummaryProcessor):
    def __init__(self, model: Callable, sentence_handler: SentenceHandler, random_state: int = 12345):
        super(CustomSummaryProcessor, self).__init__(model, sentence_handler, random_state)
    def cluster_runner(
            self,
            sentences: List[str],
            ratio: float = 0.2,
...
class BertSummarizer(CustomSummaryProcessor):
...
def do_summarize(body):
    model = Summarizer()
    result = model(body, min_length=0, num_sentences=1, ratio=0.5, use_first=True)
    full = ''.join(result)
    return full